...
ENDCOMPONENTS_HEADER = 'ENDCOMPONENTS'
COMPONENT_NAME_HEADER = 'NAME'
COMPONENT_DESCRIPTION_HEADER = 'DESCRIPTION'
    ...
    with open(specfile, 'r') as ifile:
        for line in ifile:
            ...
            # parse file header line
                ...
                if COMPONENT_HEADER in file_header or ENDCOMPONENTS_HEADER in file_header:
                    if contract.is_full():
                        # contract.saturate_guarantees()
                        component_dictionary[cgt_component.get_name()] = CGTComponent(cgt_component.get_name(), contracts=contract)
                    else:
                ...
                if CONSTANTS_HEADER in file_header:
                    ...
                    if ntabs == COMPONENT_HEADER_INDENT:
                        ...
                        if COMPONENT_NAME_HEADER in component_header:
                            cgt_component.set_name(line.strip())
                            for key, value in constants.items():
                                contract.add_constant((key, value))
                        elif COMPONENT_DESCRIPTION_HEADER in component_header:
                            cgt_component.set_description(line.strip())
                        elif CONTRACT_VARIABLES_HEADER in component_header:
                            var, init = line.split(ASSIGNMENT_CHAR, 1)
                            contract.add_variable((var.strip(), init.strip()))
                        elif CONTRACT_ASSUMPTIONS_HEADER in component_header:
                        ...
                        elif CONTRACT_GUARANTEES_HEADER in component_header:
                            list_of_variables = re.split(OPERATORS, line)
                            list_stripped = []
                            for elem in list_of_variables:
                                stripped = elem.strip()
                            ...
                            raise Exception("Unexpected Component Header")

    print("Loaded Components:\n\n____________________________________________________________________\n\n")
    for key, value in component_dictionary.items():
