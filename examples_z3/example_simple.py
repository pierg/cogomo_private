...
# a_s, c_s, d_s, f_s = symbols('a_s, c_s d_s, f_s')
#
# exp = (a_s & d_s) | ~c_s | f_s
#
# print to_cnf(exp)
...
#         g_conj.remove(x)
#
# a_comp = Or(And(a_conj), Not(And(g_conj)))
# print
#
# p1 = Bool('p_a')
# s.add(Implies(p1, a))
...
v = Bool('v')
s_gps = Bool('s_gps')
s_radar = Bool('s_radar')
s_network = Bool('s_network')
a = Bool('a')
c_ego = Bool('c_ego')
p = Bool('p')
d_rad = Bool('d_rad')
c_front = Bool('c_front')
d_front = Bool('d_front')


a_conj = [v, s_gps, s_radar, s_network, a, c_ego, p, d_rad, c_front]
g_conj = [a, c_ego, p, d_rad, c_front, d_front]
...
a_conj_simplified = a_conj[:]
g_conj_simplified = g_conj[:]
for x in a_conj:
    if x in g_conj:
        a_conj_simplified.remove(x)
        g_conj_simplified.remove(x)

a_comp = Or(And(a_conj_simplified), Not(And(g_conj_simplified)))
...
s.add(a_comp)
s.add(d_front)
s.add(Implies(vel >= 50))
s.add(Implies(vel <= 50))
s.add(Implies(vel == 60))
